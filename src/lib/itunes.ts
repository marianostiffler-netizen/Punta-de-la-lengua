 ...
 * @param {Object} analysis - Análisis de Claude con keywords, artist, etc
 * @returns {Array} Lista de canciones encontradas
 */
export async function searchITunes(analysis: any) {
  try {
    ...
    const songs = results.map((track: any) => ({
      // Información básica
 ...
 */
export async function searchByArtist(artistName: string, limit = 20) {
  try {
 ...
 */
export async function searchByAlbum(albumName: string, artistName: string | null = null) {
  try {
